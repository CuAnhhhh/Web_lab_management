...
import { getProjectReportDetail } from "src/PortalPages/api/ReportApi";
...
const ProjectReportDetail = () => {
  ...
  const [isMultiProject, setIsMultiProject] = useState<boolean>();
  ...
  useEffect(() => {
    if (
      !(projectId == userData?.projectId && userData?.isLeader) &&
  ...
  const getReportDetailFucntion = async () => {
    try {
      openLoading();
      const result = await getProjectReportDetail(projectId);
      ...
      if (result?.isDone) {
        ...
        setIsMultiProject(result?.isMultiProject);
      }
    } catch (e) {
      notification.open({
